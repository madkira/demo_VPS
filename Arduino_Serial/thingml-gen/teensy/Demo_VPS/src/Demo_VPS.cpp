...
/*****************************************************************************
 * Definitions for configuration : Demo_VPS
 *****************************************************************************/
...
// Enqueue of messages Backbone::Output::positionCoordinates
void enqueue_Backbone_send_Output_positionCoordinates(struct Backbone_Instance *_instance, double x, double y, double z){
if ( fifo_byte_available() > 28 ) {

_fifo_enqueue( (18 >> 8) & 0xFF );
_fifo_enqueue( 18 & 0xFF );

// ID of the source port of the instance
_fifo_enqueue( (_instance->id_Output >> 8) & 0xFF );
_fifo_enqueue( _instance->id_Output & 0xFF );
...
// Enqueue of messages Backbone::Output::nothing
void enqueue_Backbone_send_Output_nothing(struct Backbone_Instance *_instance){
if ( fifo_byte_available() > 4 ) {
...
if (sender == backbone_var.id_Output) {
Demo_handle_op_positionCoordinates(&demo_var, param_x, param_y, param_z);
...
//New dispatcher for messages
void dispatch_nothing(uint16_t sender) {
if (sender == calibrationStorer_var.id_progress) {
...
if (sender == backbone_var.id_Output) {
Demo_handle_op_nothing(&demo_var);
...
case 18:{
byte mbuf[28 - 2];
while (mbufi < (28 - 2)) mbuf[mbufi++] = fifo_dequeue();
uint8_t mbufi_positionCoordinates = 2;
union u_positionCoordinates_x_t {
double p;
byte bytebuffer[8];
} u_positionCoordinates_x;
u_positionCoordinates_x.bytebuffer[7] = mbuf[mbufi_positionCoordinates + 0];
u_positionCoordinates_x.bytebuffer[6] = mbuf[mbufi_positionCoordinates + 1];
u_positionCoordinates_x.bytebuffer[5] = mbuf[mbufi_positionCoordinates + 2];
u_positionCoordinates_x.bytebuffer[4] = mbuf[mbufi_positionCoordinates + 3];
u_positionCoordinates_x.bytebuffer[3] = mbuf[mbufi_positionCoordinates + 4];
u_positionCoordinates_x.bytebuffer[2] = mbuf[mbufi_positionCoordinates + 5];
u_positionCoordinates_x.bytebuffer[1] = mbuf[mbufi_positionCoordinates + 6];
u_positionCoordinates_x.bytebuffer[0] = mbuf[mbufi_positionCoordinates + 7];
mbufi_positionCoordinates += 8;
union u_positionCoordinates_y_t {
double p;
byte bytebuffer[8];
} u_positionCoordinates_y;
u_positionCoordinates_y.bytebuffer[7] = mbuf[mbufi_positionCoordinates + 0];
u_positionCoordinates_y.bytebuffer[6] = mbuf[mbufi_positionCoordinates + 1];
u_positionCoordinates_y.bytebuffer[5] = mbuf[mbufi_positionCoordinates + 2];
u_positionCoordinates_y.bytebuffer[4] = mbuf[mbufi_positionCoordinates + 3];
u_positionCoordinates_y.bytebuffer[3] = mbuf[mbufi_positionCoordinates + 4];
u_positionCoordinates_y.bytebuffer[2] = mbuf[mbufi_positionCoordinates + 5];
u_positionCoordinates_y.bytebuffer[1] = mbuf[mbufi_positionCoordinates + 6];
u_positionCoordinates_y.bytebuffer[0] = mbuf[mbufi_positionCoordinates + 7];
mbufi_positionCoordinates += 8;
union u_positionCoordinates_z_t {
double p;
byte bytebuffer[8];
} u_positionCoordinates_z;
u_positionCoordinates_z.bytebuffer[7] = mbuf[mbufi_positionCoordinates + 0];
u_positionCoordinates_z.bytebuffer[6] = mbuf[mbufi_positionCoordinates + 1];
u_positionCoordinates_z.bytebuffer[5] = mbuf[mbufi_positionCoordinates + 2];
u_positionCoordinates_z.bytebuffer[4] = mbuf[mbufi_positionCoordinates + 3];
u_positionCoordinates_z.bytebuffer[3] = mbuf[mbufi_positionCoordinates + 4];
u_positionCoordinates_z.bytebuffer[2] = mbuf[mbufi_positionCoordinates + 5];
u_positionCoordinates_z.bytebuffer[1] = mbuf[mbufi_positionCoordinates + 6];
u_positionCoordinates_z.bytebuffer[0] = mbuf[mbufi_positionCoordinates + 7];
mbufi_positionCoordinates += 8;
dispatch_positionCoordinates((mbuf[0] << 8) + mbuf[1] /* instance port*/,
 u_positionCoordinates_x.p /* x */ ,
 u_positionCoordinates_y.p /* y */ ,
 u_positionCoordinates_z.p /* z */ );
break;
}
case 19:{
byte mbuf[4 - 2];
while (mbufi < (4 - 2)) mbuf[mbufi++] = fifo_dequeue();
uint8_t mbufi_nothing = 2;
dispatch_nothing((mbuf[0] << 8) + mbuf[1] /* instance port*/);
break;
}
case 6:{
byte mbuf[4 - 2];
...
void forward_Demo_send_serial1sender_calibrating(struct Demo_Instance *_instance){
if(_instance->id_serial1sender == demo_var.id_serial1sender) {
forward_Serial1_Demo_send_serial1sender_calibrating(_instance);
}
}
void forward_Demo_send_serial1sender_positionsend(struct Demo_Instance *_instance, uint32_t x, uint32_t y, uint32_t z){
if(_instance->id_serial1sender == demo_var.id_serial1sender) {
forward_Serial1_Demo_send_serial1sender_positionsend(_instance, x, y, z);
}
}
void forward_Demo_send_serial1sender_noSignal(struct Demo_Instance *_instance){
if(_instance->id_serial1sender == demo_var.id_serial1sender) {
forward_Serial1_Demo_send_serial1sender_noSignal(_instance);
}
}
void forward_Demo_send_serial1sender_play(struct Demo_Instance *_instance){
if(_instance->id_serial1sender == demo_var.id_serial1sender) {
forward_Serial1_Demo_send_serial1sender_play(_instance);
}
}
...
void initialize_configuration_Demo_VPS() {
// Initialize connectors
register_external_Demo_send_serial1sender_positionsend_listener(&forward_Demo_send_serial1sender_positionsend);
register_external_Demo_send_serial1sender_noSignal_listener(&forward_Demo_send_serial1sender_noSignal);
register_external_Demo_send_serial1sender_calibrating_listener(&forward_Demo_send_serial1sender_calibrating);
register_external_Demo_send_serial1sender_play_listener(&forward_Demo_send_serial1sender_play);
...
register_Backbone_send_Output_positionCoordinates_listener(&enqueue_Backbone_send_Output_positionCoordinates);
register_Backbone_send_Output_nothing_listener(&enqueue_Backbone_send_Output_nothing);
...
// Init the ID, state variables and properties for instance demo
demo_var.active = true;
demo_var.id_op = add_instance( (void*) &demo_var);
demo_var.id_serial1sender = add_instance( (void*) &demo_var);
demo_var.Demo_alpha_State = DEMO_ALPHA_IDLE_STATE;

Demo_alpha_OnEntry(DEMO_ALPHA_STATE, &demo_var);
...
int main(){
	...
	initialize_configuration_Demo_VPS();
